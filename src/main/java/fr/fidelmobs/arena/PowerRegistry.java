...
public final class PowerRegistry {
    ...
    static {
        ...
        enregistrer("cercle_de_flammes", "Cercle de flammes", MobRarity.PEU_COMMUN, Material.BLAZE_POWDER,
                List.of("§7Enflamme tous les ennemis (joueurs et mobs adverses) dans un rayon de 5 blocs."),
                (plugin, joueur) -> {
        ...
        enregistrer("onde_de_recul", "Onde de recul", MobRarity.PEU_COMMUN, Material.PISTON,
                List.of("§7Repousse violemment tous les ennemis (joueurs et mobs adverses) dans un rayon de 4 blocs."),
                (plugin, joueur) -> {
                    UUID lanceur = joueur.getUniqueId();
        ...
        // ---- RARE : nécessitent de viser un adversaire (joueur ou mob adverse) ----
        enregistrer("echange_spectral", "Échange spectral", MobRarity.RARE, Material.ENDER_PEARL,
                List.of("§7Échange ta position avec le joueur ou mob adverse visé."),
                (plugin, joueur) -> {
                    LivingEntity cible = ciblerVivant(plugin, joueur, 25);
                    if (cible == null) {
                    ...
                    joueur.getWorld().playSound(joueur.getLocation(), Sound.ENTITY_ENDERMAN_TELEPORT, 1f, 1f);
                    joueur.getWorld().spawnParticle(Particle.PORTAL, joueur.getLocation().add(0, 1, 0), 40, 0.4, 0.6, 0.4, 0.3);
                    cible.getWorld().playSound(cible.getLocation(), Sound.ENTITY_ENDERMAN_TELEPORT, 1f, 1f);
                    cible.getWorld().spawnParticle(Particle.PORTAL, cible.getLocation().add(0, 1, 0), 40, 0.4, 0.6, 0.4, 0.3);
                });

        enregistrer("grappin_tenebreux", "Grappin ténébreux", MobRarity.RARE, Material.LEAD,
                List.of("§7Attire violemment le joueur ou mob adverse visé vers toi."),
                (plugin, joueur) -> {
        ...
        enregistrer("ralliement", "Ralliement", MobRarity.RARE, Material.LEAD,
                List.of("§7Convertit le mob adverse visé en allié à ta solde (jamais un joueur)."),
                (plugin, joueur) -> {
                    LivingEntity cible = ciblerVivant(plugin, joueur, 15);
                    if (!(cible instanceof Mob mobCible)) {
                        joueur.sendMessage("§cVise un mob adverse, pas un joueur.");
                        return;
                    }
                    plugin.getAllyListener().changerProprietaire(mobCible, joueur);
                    mobCible.getWorld().playSound(mobCible.getLocation(), Sound.ENTITY_VILLAGER_YES, 1f, 1f);
                    mobCible.getWorld().spawnParticle(Particle.HEART, mobCible.getLocation().add(0, 1, 0), 25, 0.3, 0.5, 0.3, 0);
                    joueur.sendMessage("§a✦ " + mobCible.getType().name() + " a rejoint ta cause !");
                });
        ...
        enregistrer("aura_dissipation", "Aura de dissipation", MobRarity.RARE, Material.SHULKER_SHELL,
                List.of("§7T'immunise contre TOUS les pouvoirs adverses pendant 15 secondes."),
                (plugin, joueur) -> {
                    accorderImmunitePouvoirs(joueur.getUniqueId(), 15_000L);
                    joueur.getWorld().playSound(joueur.getLocation(), Sound.ITEM_SHIELD_BLOCK, 1f, 0.5f);
                    joueur.getWorld().spawnParticle(Particle.END_ROD, joueur.getLocation().add(0, 1, 0), 50, 0.4, 0.9, 0.4, 0.05);
                });

        // ---- EPIQUE : forte gêne/dégâts infligés à la cible visée (joueur ou mob adverse) ----
        enregistrer("levitation_forcee", "Lévitation forcée", MobRarity.EPIQUE, Material.PHANTOM_MEMBRANE,
                List.of("§7Fait léviter le joueur ou mob adverse visé pendant 4 secondes."),
                (plugin, joueur) -> {
                    LivingEntity cible = ciblerVivant(plugin, joueur, 20);
                    if (cible == null) {
                        joueur.sendMessage("§cAucune cible visée.");
        ...
        enregistrer("foudre_ciblee", "Foudre ciblée", MobRarity.EPIQUE, Material.TRIDENT,
                List.of("§7Frappe la foudre sur le joueur ou mob adverse visé (dégâts, pas de feu)."),
                (plugin, joueur) -> {
                    LivingEntity cible = ciblerVivant(plugin, joueur, 25);
                    if (cible == null) {
                        joueur.sendMessage("§cAucune cible visée.");
        ...
        enregistrer("entrave_glaciale", "Entrave glaciale", MobRarity.EPIQUE, Material.PACKED_ICE,
                List.of("§7Lenteur IV, Faiblesse II et Cécité pendant 4 secondes à la cible (joueur ou mob adverse)."),
                (plugin, joueur) -> {
                    LivingEntity cible = ciblerVivant(plugin, joueur, 20);
                    if (cible == null) {
                        joueur.sendMessage("§cAucune cible visée.");
        ...
        enregistrer("pluie_de_meteores", "Pluie de météores", MobRarity.LEGENDAIRE, Material.FIRE_CHARGE,
                List.of("§7Fait pleuvoir des météores autour du joueur ou mob adverse visé."),
                (plugin, joueur) -> {
                    UUID lanceur = joueur.getUniqueId();
                    LivingEntity cible = ciblerVivant(plugin, joueur, 25);
                    ...
                    for (int i = 0; i < 6; i++) {
                        ...
                        plugin.getServer().getScheduler().runTaskLater(plugin, () -> {
                            ...
                            for (Entity proche : monde.getNearbyEntities(impact, 2, 3, 2)) {
                                if (proche instanceof LivingEntity touche && estCibleValide(plugin, lanceur, touche)) {
                                    touche.damage(4.0);
        ...
        // ---- COMMUN ----
        // (regard_predateur supprimé : jugé peu utile)
    ...
    // Immunité temporaire "anti-pouvoirs" (voir le pouvoir "Aura de dissipation") : pendant
    // sa durée, ce joueur ne peut être affecté par AUCUN pouvoir, quel qu'en soit le lanceur.
    // Ne concerne que les pouvoirs (cette carte), jamais les dégâts de combat classiques.
    private static final Map<UUID, Long> IMMUNITE_POUVOIRS_JUSQUA = new HashMap<>();

    public static void accorderImmunitePouvoirs(UUID uuid, long dureeMs) {
        IMMUNITE_POUVOIRS_JUSQUA.put(uuid, System.currentTimeMillis() + dureeMs);
    }

    private static boolean estImmuniseContrePouvoirs(UUID uuid) {
        Long jusqua = IMMUNITE_POUVOIRS_JUSQUA.get(uuid);
        return jusqua != null && System.currentTimeMillis() < jusqua;
    }
    ...
    private static boolean estCibleValide(LoyaltyMobsPlugin plugin, UUID lanceur, LivingEntity cible) {
        if (cible instanceof Player p) {
            if (estImmuniseContrePouvoirs(p.getUniqueId())) return false;
